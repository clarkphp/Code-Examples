...
if (empty($args)) { // could be wise to validate contents of each array element
    ZendJobQueue::setCurrentJobStatus(ZendJobQueue::FAILED, 'B; No output filepath supplied');
    return -1;
}
...
for ($i = 0; $i < 5; $i++) {
    if (!writeData($filepath, $i)) {
        ZendJobQueue::setCurrentJobStatus(ZendJobQueue::FAILED, 'B: Failure writing to output file');
        return -1;
    }
}

ZendJobQueue::setCurrentJobStatus(ZendJobQueue::OK, 'B: Finished. See ' . $args['output_file_name']);

function writeData($outputFilepath, $datum) {
    sleep(1); // simulate taking some time to work
    if (false === file_put_contents(
        $outputFilepath,
        date('Y-m-d H:i:s') . ' - B: iteration ' . $datum . PHP_EOL,
        FILE_APPEND
